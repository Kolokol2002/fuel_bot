...
@dp.message_handler(Text(equals='✅Км', ignore_case=True))
async def km_1(message: types.Message):
...
async def km_2(message: types.Message, state: FSMContext):
    if message.text.isdigit():
        list_cost_middle = await sqlite_db.sql_give_cost_and_middle(message)
    ...
    @dp.callback_query_handler(text='back', state=For_Answer.km)
...
@dp.message_handler(Text(equals='✅Грн', ignore_case=True))
async def rgn_1(message: types.Message):
    ...
    @dp.callback_query_handler(text='back', state=For_Answer.grn)
    async def next_keyboard(call: types.CallbackQuery, state:FSMContext):
        await call.message.answer(text='🔙Назад', reply_markup=kb)
